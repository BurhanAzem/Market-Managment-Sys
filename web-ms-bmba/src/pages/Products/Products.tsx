...
import { useAppSelector, useAppDispatch } from '../../redux/store/hooks'; // Custom hooks
import { getProducts, openAddProductModal } from '../../redux/features/productSlice';
...
const Products: FC = (): ReactElement => {
  ...
  const { products, loading, error, currPageNumber, totalPages } = useAppSelector((state: { product: any; }) => state.product); // Get product state

  React.useEffect(() => {
    dispatch(getProducts({ pageNumber: 1, pageSize: 10 })); // Fetch products when the component mounts
  }, [dispatch]);
  ...
  //   (state: RootState) => state.product.products);
    // const currPageNumber = useAppSelector((state: { product: any; }) => state.product.pageNumber); // Get product state
    // const totalPages = useAppSelector((state: { product: any; }) => state.product.totalPages); // Get product state
  ...
  const RowToCheckedList = (row: any) => {
    setCheckedRows((prevCheckedRows: any[]) => {
      ...
      if (isChecked) {
        ...
        return prevCheckedRows.filter((code: any) => code !== row.barCode);
      } else {
    ...
    setCheckedRows((prevCheckedRows: any[]) => {
      ...
      if (isChecked) {
        ...
        return prevCheckedRows.filter(
          (          code: any) => ![...products.map((prod: { barCode: any; }) => prod.barCode)].includes(code)
        );
      } else {
        // Add the barCode to checkedRows
        return [...prevCheckedRows, ...products.map((prod: { barCode: any; }) => prod.barCode)];
      }
    });
  };
            ...
            loading ?
              ...
              <Table sx={{ minWidth: 650, color: 'black', width: '100%' }} aria-label="simple table">
                ...
                <TableBody>
                  ...
                  {products && products.map((row: { barCode: any; name: any; categoryDto: { name: any; }; quantityOfProductsPresentedForRetail: any; quantityOfProductsPresentedForWholesale: any; currentRetailSellingPrice: any; }) => (
                    <TableRow
                      key={row.barCode}
